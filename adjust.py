...
for i in range(len(jiedian)-1):
    if i%10000==0:
        print(i)
    copy=yuanru[jiedian[i]:jiedian[i+1]]
    ...
    leftright=np.zeros(len(a),dtype=[("PETime",np.int),("Weight",np.float)])
    k=0
...
#    while True:
#        newp=np.argmin(leftright["Weight"])
#        if leftright[newp]["Weight"]<-0.5:
#            minustime.append(leftright[newp]["PETime"])
#            leftright[newp]["Weight"]=0
#            if leftright[newp+1]["PETime"]==leftright[newp]["PETime"]+1:
#                leftright[newp+1]["Weight"]=0
#            if leftright[newp-1]["PETime"]==leftright[newp]["PETime"]-1:
